...
> 更新日期：2026-07-20
>
> 目前狀態：`0.1.0` 未簽章公開測試版已發布；`0.2.0` PDF 轉圖片功能已由使用者確認正常，正在完成跨版本升級驗收。
...
| 自動化測試 | 已完成，主機環境受限 | 目前共 74 項；本次 70 項通過，新增的升級驗證測試亦通過，另 4 項既有 Streamlit UI／自我檢查測試因本機 Application Control 阻擋未簽章 `pyarrow` DLL 而無法執行，並非測試斷言失敗 |
| 瀏覽器介面驗證 | 已完成 | 本機以六份不同方向 PDF 驗證跨列拖曳、移除、合併及下載；外部 Windows 安裝版人工驗收亦通過 |
| PDF 渲染驗證 | 已完成 | 直向、橫向、旋轉頁面、合併交界與 PDFium 第一頁縮圖均已渲染檢查，方向與內容正確 |
| Git、GitHub repository 與 0.1.0 Release | 已完成 | `main`、`v0.1.0` tag、一般 GitHub Release、兩個下載資產、latest 連結與 HTTPS 更新資訊均已驗證 |
...
| 發布驗收腳本 | 已完成 | 單版驗收涵蓋安裝、未自動啟動、`--self-test`、GUI、服務、loopback、背景程序及解除安裝；另新增 0.1.0 → 0.2.0 原地覆蓋升級、單一解除安裝紀錄與清理的自動驗證腳本 |
| 無 Python 電腦驗證 | 已完成 | 使用者回報外部 Windows 環境的安裝、啟動、操作及解除安裝檢查皆正常 |
| 更新機制 | 進行中 | 已改為 HTTPS 檢查版本並開啟 GitHub Releases 手動下載；原地覆蓋升級自動驗證已實作，待乾淨安裝狀態實跑及 0.2.0 發布後驗證提示 |
| PDF 轉圖片 | 已實作，待外部環境資訊 | 多 PDF、PNG／JPEG、兩種 ZIP 結構、命名、瀏覽器、代表性渲染與 0.2.0 候選打包均已驗證；使用者已回報新增功能正常，但尚未記錄本次電腦是否為無 Python 外部環境 |
...
`0.1.0` 已完成發布。後續維持公開更新資訊仍指向可下載的 0.1.0，開發 PDF 轉圖片，待 0.2.0 Release 實際存在後才更新公開 feed，並驗證跨版本更新提示與手動覆蓋安裝。
...
| 7 | `feature/pdf-to-images` | 多 PDF 逐頁轉 PNG／JPEG 與 0.2.0 更新驗證 | 兩種 ZIP 結構、命名、渲染、記憶體釋放、打包與跨版本更新均通過驗收 |
...
4. **已完成候選功能驗收**：PDF 轉圖片功能、瀏覽器、代表性渲染及 0.2.0 候選打包已通過，使用者亦回報新增功能正常；仍待補記無 Python 外部環境結果。
5. **已完成腳本、待乾淨狀態實跑**：`verify-upgrade.ps1` 會核對兩版安裝程式與 SHA-256、乾淨安裝 0.1.0、直接覆蓋 0.2.0、驗證兩版 `--self-test`、固定安裝位置、單一解除安裝紀錄及最後清理。目前開發電腦已有 0.2.0 正在執行，安全前置檢查會拒絕覆蓋現有安裝。
