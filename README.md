# PDF 合併工具（本機 PDF 工具箱原型）

這是一個以 Streamlit 與 pypdf 製作的本機 PDF 合併工具。它可以調整檔案順序、合併並下載結果；所有 PDF 都只在程式記憶體中處理。

目前完成合併 PDF 原型；桌面啟動器、離線安裝程式、更新機制與拆分 PDF 尚在規劃中。詳細資訊請參閱 [專案文件](docs/README.md)。
