...
| 自動化測試 | 已完成 | 48 個核心、預覽資源釋放、容量邊界、完整合併互動、介面、啟動器、更新與打包設定測試通過；封裝清單與所有自家 Python 模組一致 |
...
| PyInstaller onedir | 進行中 | Python 3.13.14 建置成功並核對 14 個必要模組、PDFium DLL 及授權檔；最新完整建置直接通過強化後的 `--self-test` 與 loopback smoke test，待乾淨環境 GUI 驗收 |
...
- 主程式新增 `--self-test`，實際執行封裝內首頁與合併頁的 Python 程式，建立兩張 PDF 卡片並載入拖曳元件後端，再驗證代表性 PDF 讀取、第一頁渲染及合併。它不能取代一般瀏覽器中的滑鼠拖曳與畫面人工驗收。
