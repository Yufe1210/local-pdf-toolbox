...
> 更新日期：2026-07-24
>
> 目前狀態：`0.2.1` 空密碼 PDF 相容性修正未簽章公開測試版已發布。
...
| Git、GitHub repository 與 Releases | 已完成 | `v0.1.0`、`v0.2.0`、`v0.2.1` tag、一般 GitHub Releases、版本化下載資產、latest 連結與 HTTPS 更新資訊均已建立 |
...
| Inno Setup 安裝程式 | 已完成候選驗收 | 0.2.1 版本化安裝程式與 SHA-256 已建立；本機封裝、外部 Windows 安裝及 0.2.0 → 0.2.1 覆蓋升級驗收均通過 |
| 發布驗收腳本 | 已完成 | 0.2.0 單版、0.1.0 → 0.2.0，以及 0.2.1 單版與 0.2.0 → 0.2.1 原地覆蓋升級、單一解除安裝紀錄與清理均通過 |
| 無 Python 電腦驗證 | 已完成 | 使用者回報外部 Windows 環境的安裝、啟動、操作及解除安裝檢查皆正常 |
| 更新機制 | 已完成 | HTTPS feed 已切換至 0.2.1，開啟 GitHub Releases 手動下載；0.2.0 → 0.2.1 覆蓋升級驗收通過 |
| PDF 轉圖片 | 已完成並發布 | 多 PDF、PNG／JPEG、兩種 ZIP 結構、命名、瀏覽器、代表性渲染、0.2.0 打包及外部 Windows 驗收均已完成 |
| 空密碼 PDF 相容性 | 已完成並發布 | 空密碼解鎖成功即允許處理且不檢查權限旗標；真正需要非空密碼仍拒絕；實際文件、候選封裝、外部 Windows 驗收及 0.2.1 發布均已完成 |
...
`0.2.1` 已完成發布。公開更新資訊指向可下載的 0.2.1，0.1.0 與 0.2.0 使用者可由啟動器前往 GitHub Releases 手動下載並直接覆蓋安裝；0.2.1 不會重複提示。
...
| 8 | `fix/empty-password-pdfs` | 修正空密碼權限保護 PDF 的誤判並準備 0.2.1 | **已完成**；核心、實際文件、76 項測試、完整候選打包、外部 Windows 安裝及覆蓋升級驗收均通過 |
...
7. **已完成**：0.2.1 空密碼 PDF 修正的完整測試、候選打包、外部 Windows 安裝及 0.2.0 → 0.2.1 覆蓋升級驗收均通過。
8. **已完成**：已驗收修正已合併至 `main`；`v0.2.1` tag、一般 GitHub Release、安裝程式與 SHA-256 已發布並重新下載核對，公開 feed 已切換至 0.2.1。
