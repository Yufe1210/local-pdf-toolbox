...
這是一個以 Streamlit、pypdf 與 pypdfium2 製作的本機 PDF 工具箱。目前可以檢查每份 PDF 的第一頁預覽，以拖曳或按鈕調整順序、合併並下載結果；所有 PDF 都只在程式記憶體中處理。
...
啟動後，Streamlit 會顯示本機網址。加入 PDF 後，以卡片縮圖確認文件，用滑鼠拖曳或箭頭調整順序、輸入輸出檔名，再按下「合併 PDF」。
