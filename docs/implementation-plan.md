...
| 合併介面預覽與單一狀態 | 尚未開始 | 已確認改為單一 `pdf_items`、第一頁縮圖與拖曳排序；保留上移／下移替代操作 |
...
| 發布驗收腳本 | 進行中 | 現有安裝、啟動、loopback、結束、背景程序與解除安裝驗證可用；待加入未自動啟動檢查與安裝版 `--self-test` |
...
| 4 | `feature/merge-preview` | 單一上傳狀態、PDF 卡片、第一頁縮圖、拖曳排序、關閉頁面狀態與驗收自我測試 | 排序與上傳顯示一致；縮圖僅在記憶體低解析度逐份產生；封裝與乾淨 Windows 驗收通過 |
...
目前開發電腦的 Windows 應用程式控制政策會攔截新產生的未簽章執行檔。因此本機可以建立未簽章公開測試版，但不能在這台電腦完成封裝後啟動驗收。必須改在未啟用相同封鎖政策的乾淨 Windows 電腦驗收；若目標電腦也直接封鎖，0.1.0 不得宣稱支援該環境。
...
- 新增關閉通知，使瀏覽器端在服務停止前切換為「工具已關閉」狀態；不嘗試強制關閉一般瀏覽器分頁。
